...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/01/19 19:06:19 by pgurn             #+#    #+#             */
/*   Updated: 2021/01/20 22:30:07 by pgurn            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	x_minflag2(unsigned long long x, int numlen, char *num)
{
	...
	g_n = g_info.width - (g_pre + g_zeronum + numlen);
	while (g_n > 0)
	{
		PRINT(' ');
		g_n--;
...
void		x_minflag(unsigned long long x, int var, int numlen, char *num)
{
	if (g_info.flags[4] == '#' && x != 0)
	{
		...
		g_pre = 0;
	g_zeronum = g_info.precision - numlen;
	if (g_zeronum > 0)
	{
		g_n = g_zeronum;
		while (g_n > 0)
...
void		x_zeroflag(unsigned long long x, int var, int numlen, char *num)
{
	if (g_info.flags[4] == '#' && x != 0)
	{
		...
		g_pre = 0;
	g_zeronum = g_info.width - (g_pre + numlen);
	if (g_zeronum > 0)
		while (g_zeronum > 0)
		{
			PRINT('0');
